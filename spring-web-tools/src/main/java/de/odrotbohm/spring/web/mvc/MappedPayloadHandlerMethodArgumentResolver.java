...
public class MappedPayloadHandlerMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    private <T> T getArgumentResolver(Class<T> type) {
        return adapter.get().getArgumentResolvers().stream().filter(type::isInstance).findFirst().map(type::cast).orElseThrow(() -> new IllegalArgumentException(String.format("Could not find resolver of type ", type.getName())));
    }
